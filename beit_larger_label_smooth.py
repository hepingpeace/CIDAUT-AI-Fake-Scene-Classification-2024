...
def train_one_epoch(model, train_loader, criterion, optimizer, scheduler, device, label_smoothing=0.05):
    model.train()
    running_loss = 0.0
    ...
    for images, labels in tqdm(train_loader):
        ...
        loss = criterion(outputs, smoothed_labels)  # Use smoothed labels
...
def train_model(csv_file, img_dir, model, model_name, img_size=(224, 224), num_epochs=10, batch_size=32, lr=1e-4, n_fold=0, device='cuda', patience=5, warmup_epochs=0, label_smoothing=0.05):
    ...
    for epoch in range(num_epochs):
        ...
        train_loss = train_one_epoch(model, train_loader, criterion, optimizer, scheduler, device, label_smoothing)
...
if __name__ == "__main__":
    ...
    train_model(labels, img_dir, model, '/home/work/COOOL/faker_image/beit-large-patch16-384', img_size=img_size, num_epochs=50, 
                batch_size=batch_size, lr=lr, n_fold=n_fold, patience=5, warmup_epochs=0)
